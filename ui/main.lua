-- remake by pluem4898 | UX/UI by biggaboy212
...
function MacLib:Window(Settings)
	...
	baseUICorner.CornerRadius = UDim.new(0, 15)
	...
	baseUIStroke.Transparency = 0.95
	...
	local ResizeHandleInput = Instance.new("ImageLabel")
	ResizeHandleInput.Name = "Input"
	ResizeHandleInput.Image = "rbxassetid://132200097370077"
	ResizeHandleInput.Size = UDim2.fromScale(1, 1)
	ResizeHandleInput.BackgroundTransparency = 1
	ResizeHandleInput.ImageColor3 = Color3.fromRGB(255, 255, 255)
	ResizeHandleInput.ImageTransparency = 0.2
	ResizeHandleInput.Parent = ResizeHandle
	...
	windowControls.Size = UDim2.new(1, 0, 0.05, 0)
	...
	elseif Settings.DragStyle == 2 then
		base.InputBegan:Connect(function(input)
			...
			UserInputService.InputChanged:Connect(function(input)
				if input == dragInput and dragging_ and not isResizing then
					update(input)
				end
			end)
			...
			local function SelectCurrentTab()
				...
				for _, v in pairs(tabSwitchersScrollingFrame:GetDescendants()) do
					if v.Name == "TabSwitcher" then
						...
						local tabName = v:FindFirstChild("TabSwitcherName")
						if tabName then
							Tween(tabName, TweenInfo.new(easetime, Enum.EasingStyle.Sine), {
						...
						if tabImg then
							Tween(tabImg, TweenInfo.new(easetime, Enum.EasingStyle.Sine), {
				...
				local tabName = tabSwitcher:FindFirstChild("TabSwitcherName")
				if tabName then
					Tween(tabName, TweenInfo.new(easetime, Enum.EasingStyle.Sine), {
						TextColor3 = Color3.fromRGB(30, 110, 244),
						TextTransparency = 0.0
					}):Play()
				end

				local tabImg = tabSwitcher:FindFirstChild("TabImage")
				if tabImg then
					Tween(tabImg, TweenInfo.new(easetime, Enum.EasingStyle.Sine), {
						ImageColor3 = Color3.fromRGB(30, 110, 244),
	...
	UserInputService.InputChanged:Connect(function(input)
		if isResizing and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
			...
			local newWidth = initialSize.X.Offset + delta.X
			local newHeight = initialSize.Y.Offset + delta.Y
